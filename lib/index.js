...
class Aekatva {
    ...
    uuid_v2(domain, identifier) {
        ...
        const timeLow = ((time & 0xffffffff) >>> 0).toString(16).padStart(8, '0');
        const timeMid = ((time >>> 32) & 0xffff).toString(16).padStart(4, '0');
        const timeHi = ((time >>> 48) & 0x0fff) | 0x2000; // Version 2
        const clockSeqHigh = ((r(1)[0] & 0x3f) | 0x80).toString(16).padStart(2, '0');
        const clockSeqLow = r(1)[0].toString(16).padStart(2, '0');
        const nodePart1 = (domain & 0xf).toString(16); // Ensuring domain is a single hex digit
        const nodePart2 = (identifier & 0xfffffffffff).toString(16).padStart(12 - nodePart1.length, '0'); // Padding to ensure 12 characters
    ...
    uuid_v3(namespace, name) {
        ...
        if (typeof window !== 'undefined' && window.crypto) {
            ...
            throw new Error('Aekatva: uuid v3 generation is not supported in this environment.');
        }
    ...
    uuid_v4(){
        ...
        if (typeof window !== 'undefined' && (window.crypto || window.msCrypto)) {
            ...
            throw new Error('Aekatva: uuid v4 generation is not supported in this environment.');
        }

        const parts = [
            r(4).map((byte) => byte.toString(16).padStart(2, '0')).join(''),
            r(2).map((byte) => byte.toString(16).padStart(2, '0')).join(''),
            '4' + r(2).map((byte) => byte.toString(16).padStart(2, '0')).join('').slice(-3), // set version (4)
            '8' + r(2).map((byte) => byte.toString(16).padStart(2, '0')).join('').slice(-3), // set variant
